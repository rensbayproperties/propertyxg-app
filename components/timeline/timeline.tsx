...
>(({ className, ...props }, ref) => (
	<ol ref={ref} className={cn("flex flex-col overflow-hidden", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
	<div
		ref={ref}
		className={cn(
			"absolute top-[5px] left-[30px] -translate-x-1/2 translate-y-2 h-full w-px  bg-brand",
			className,
		)}
		{...props}
	/>
...
>(({ className, ...props }, ref) => (
	<div
		ref={ref}
		className={cn("flex flex-col size-3 bg-brand rounded-full", className)}
		{...props}
	/>
));
