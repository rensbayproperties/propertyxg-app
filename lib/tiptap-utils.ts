...
export const handleImageUpload = async (
  ...
  try {
    const upload = await axiosAuth.post("/media/upload/multiple-stateless", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    })

    return upload?.data?.data?.urls && upload.data.data.urls?.length > 0 && upload.data.data.urls[0] || ""
  } catch (error) {
    return ""
  }
}
